...
/obj/item/reagent_containers/food/snacks/corpse_cube/proc/CorpseExpand(var/datum/source_DNA,var/datum/source_name,var/datum/source_species,var/datum/source_pronouns,var/datum/source_blood)
	var/datum/target_match
	for(var/datum/computer_file/report/crew_record/CR in GLOB.all_crew_records)
		var/blood_dna = CR.get_dna()
		src.visible_message(SPAN_WARNING("[blood_dna]"))
		if (blood_dna == source_blood)
			target_match = CR
			break

			//var/gender = CR.get_gender()
	var/name = target_match.get_name()
	src.visible_message(SPAN_WARNING("[name]"))
	//var/species = target_match.get_species()
	//var/pronouns = target_match.get_sex()
	src.visible_message(SPAN_WARNING("Found match in crew records by the identifier of [name]"))
	if(!growing)
		growing = 1
		var/mob/living/carbon/human/H = new spawn_type
		H.dna.uni_identity = source_DNA
...
/obj/item/reagent_containers/food/snacks/corpse_cube/on_reagent_change()
	...
	if(reagents.has_reagent(/datum/reagent/blood))
		...
		sleep(2)
		CorpseExpand(target_DNA,target_name,target_species,target_pronouns,blood_MD5)
